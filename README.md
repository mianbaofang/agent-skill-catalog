...
A local Skill catalog for Codex and other AI coding agents. It works as a practical Codex Skill organizer: scan explicit `SKILL.md` roots, keep parent and child Skills together, list plugins separately, and show how each entry is invoked.
...
| GitHub URLs | A repository link appears only when frontmatter, a local Git remote, a manifest, or reviewed curation provides it. Clicking a preview opens that repository. |
...
When a scanned Skill has an observed GitHub repository, the default build caches one public repository preview. Add `--no-github-images` to `build_catalog.py` for a fully offline build. In server mode, the detail view can replace a preview manually and later restore the automatic image.
...
- Current published release: [`v0.2.2`](https://github.com/mianbaofang/agent-skill-catalog/releases/tag/v0.2.2)
